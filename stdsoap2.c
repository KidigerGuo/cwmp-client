...
soap_begin_serve(struct soap *soap)
{
  ...
  { if (soap->error < SOAP_STOP)
	...
	if (soap->error == SOAP_NO_DATA || soap->error == 200) // this use to handle empty post from client .
	{
#ifndef SUPPORT_EMPTYPOST_REQ
		soap->keep_alive = 0 ;
#ifdef WITH_FASTCGI
		soap_send_empty_response(soap, 204); //send empty post 204 to terminal the session .
#else
		return soap_send_empty_response(soap, 204);
#endif
#else
#ifdef WITH_FASTCGI
		__cwmp_soap_emptypost_handle(soap);
#else
		//soap->error = 200 ;
		return __cwmp_soap_emptypost_handle(soap); // need add response api here .
#endif
#endif
	}
	else
...
#endif
#ifdef SUPPORT_EMPTYPOST
    if(!soap->body) // due to Empty post no Http body , so we need return before check soap-body .
